...
[![Release](https://github.com/IcyShadow5/godot-coder-ai/actions/workflows/release.yml/badge.svg)](https://github.com/IcyShadow5/godot-coder-ai/actions/workflows/release.yml)
...
- **The honest part:** a 91M model after roughly one pass over the data does *not* write working GDScript yet. The early corpus benchmark scored 6.25% (1 of 16 prompts); once the harness graded scaffold + completion properly, the real golden baseline is **2/30 (6.7 %)** at temperature 0 — a baseline, not a result. After the repetition-penalty sampling work in v0.10.22 the same checkpoint scores **6/30 (20 %)**. What it *did* learn is telling: it reproduces the exact task-header format of the training data, which means the pipeline, tokenizer and training loop are real.
...
- **v0.10.21** - test-coverage round that found one real bug:
  ...
  tests cover the training/corpus router handlers, doctor and profile
  probe. 465 tests.
...
- **v0.10.14** - robustness from an external review. `prepare_dataset`
  ...
  watchdog test is fixed. Cleanup: unused imports, semicolon chains
  split in train.py, LF line endings, broader secret masking (GitHub
  tokens, JWTs, connection strings). 254 tests.
