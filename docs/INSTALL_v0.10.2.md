# Installation and upgrade to v0.10.2
...
`<Project>\.upgrade_backups\v0.10.2-<timestamp>`. In case of a
...
See `docs/CHANGELOG_v0.10.2.md`. In short: The secret scan now runs in every
mode (security fix), `SKIP_PROJECT_IMPORT` wins over `FAST_STATIC`,
`validate_dataset.py` uses the managed-process runner + env timeout, Studio toggles
for the fast import; the upgrade tooling lives as a template in the repo (`upgrade/`).
